...
# --- C Function Declarations (MODIFIED) ---
...
# --- Python Wrapper Classes (Unchanged) ---
...
cdef class ChessMove:
    cdef CChessMove c_move; cdef BoardPosition _start_pos_py; cdef BoardPosition _target_pos_py
...
cdef class ChessEngine:
    ...
    def __cinit__(self, str library_path, str model_path):
        # (Constructor unchanged from previous version - allocates buffer)
        self.c_engine_handle = NULL; self._handle_valid = False
        self.c_moves_buffer = NULL; self.c_moves_buffer_capacity = 256
        ...
        try:
            ...
            if self.c_moves_buffer == NULL:
                ...
                if self._handle_valid: with nogil: engine_destroy(self.c_engine_handle)
            ...
            if self.c_moves_buffer != NULL: free(self.c_moves_buffer); self.c_moves_buffer = NULL
            if self._handle_valid and self.c_engine_handle != NULL: with nogil: engine_destroy(self.c_engine_handle)
            self.c_engine_handle = NULL; self._handle_valid = False
            raise

    def __dealloc__(self):
        # (Destructor unchanged - frees buffer)
        if self._handle_valid and self.c_engine_handle != NULL:
            ...
            self.c_engine_handle = NULL; self._handle_valid = False
        if self.c_moves_buffer != NULL:
            print(f"Cython ChessEngine: Freeing internal move buffer at {<ptrdiff_t>self.c_moves_buffer}.")
            free(self.c_moves_buffer)
            self.c_moves_buffer = NULL

    def _check_handle(self):
        # (Check handle unchanged - also checks buffer)
    ...
    def board_state_address(self):
        ...
        if c_state_ptr == NULL: return 0
        return <ptrdiff_t>c_state_ptr
    ...
    def get_valid_moves_address_count(self):
        ...
        with nogil:
            num_moves_found = engine_get_valid_moves(
                self.c_engine_handle, self.c_moves_buffer, self.c_moves_buffer_capacity)
        if num_moves_found > self.c_moves_buffer_capacity:
            print(f"[WARNING CYTHON] Moves found ({num_moves_found}) exceeds buffer capacity ({self.c_moves_buffer_capacity}). Buffer truncated.", file=sys.stderr)
    ...
    def apply_move(self, ptrdiff_t move_address) -> ptrdiff_t:
        ...
        Returns the address (int) of the new board state, or 0 on failure.
        """
        self._check_handle()
        ...
        cdef CBoardState* new_state_ptr = NULL # Pointer to receive result

        with nogil:
            # Call MODIFIED C API function
            new_state_ptr = engine_apply_move(self.c_engine_handle, move_ptr)

        if new_state_ptr == NULL:
             # C API returning NULL indicates failure
             raise ValueError("engine_apply_move returned NULL (invalid move or internal error).")

        # Return the address of the new state as an integer
        return <ptrdiff_t>new_state_ptr

    def ai_move(self, int difficulty) -> ptrdiff_t:
        """
        Triggers the AI calculation (blocking).
        Returns the address (int) of the new board state, or 0 on failure.
        """
        self._check_handle()
        ...
        try:
            with nogil:
                 # Call MODIFIED C API function
                 new_state_ptr = engine_ai_move(self.c_engine_handle, difficulty, NULL)

            if new_state_ptr == NULL:
                 print("[WARNING CYTHON] engine_ai_move returned NULL.", file=sys.stderr)
                 return 0 # Return 0 address on failure
    ...
    def move_to_str(self, ptrdiff_t move_address):
        # (Unchanged - already accepts address)
        self._check_handle()
        ...
        try:
            ...
            if not success:
                # print("[Warning CYTHON move_to_str] failed/buffer too small.", file=sys.stderr)
