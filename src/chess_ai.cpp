...
namespace chess::ai {
    ...
    void chess_ai_state::make_move(board_state& board, std::int32_t difficulty, previous_board_states& history) {
        std::cerr << "[DEBUG AI make_move] Function Entered." << std::endl << std::flush;
        search_cancelled.store(false);
        { std::lock_guard<std::mutex> lock(transposition_table_mutex); transposition_table.clear(); }
        std::cout << "[INFO] Transposition Table cleared for new search." << std::endl << std::flush;
        ...
        std::cout << "[INFO] AI (" << (board.current_player == player::white ? "White" : "Black") << ") searching at depth: " << search_depth << " (Difficulty: " << difficulty << ")" << std::endl << std::flush;
        ...
        std::cout << "[INFO] Searching " << scored_root_moves.size() << " root moves..." << std::endl << std::flush;
        ...
        if (best_move_found) {
            std::cout << "[INFO] Best move found: ("
                     << static_cast<int>(best_move.start_position.rank) << "," << static_cast<int>(best_move.start_position.file) << ")->"
                      ...
                      << " with score: " << best_score << std::endl << std::flush;
            std::cout << "[INFO] Applying best move." << std::endl << std::flush;
            std::cout << "[DEBUG AI make_move] Current player before move apply: " << static_cast<int>(board.current_player) << std::endl;
            chess::apply_move(board, best_move, history); // Apply best found move
            std::cout << "[DEBUG AI make_move] Current player after move apply: " << static_cast<int>(board.current_player) << std::endl;
        } else {
            ...
            if (!scored_root_moves.empty()) {
                ...
                if (fallback_non_gameplay != valid_moves.end()) {
                    ...
                    std::cerr << "[DEBUG AI make_move] AFTER final apply_move (fallback). Board player = " 
                              << static_cast<int>(board.current_player) << std::endl << std::flush;
                } else {
